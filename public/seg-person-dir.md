...
※ここでいう向きは、一般的な細かい角度推定ではなく2クラス（2値分類）です。

* 同方向：背中向き。カメラから遠ざかる向き
* 逆方向：正面向き。カメラに近づく向き


今回使用しているソースコードは[こちら](https://github.com/DaichiKiyozaki/seg-person-dir)です。
...
過去のYOLOモデルと比較して、性能も向上していることが分かります。
...
今回やりたいのは「同方向歩行者/逆方向歩行者」という独自ラベルなので、ファインチューニングで学習させます。
...
また、COCOはカテゴリで `person` を指定して抽出でき、さらにキーポイントで色々フィルタリングできるので、今回の要件（歩行者をセグメントしつつ前後の向きラベルを作る）に完全に合致します。
...
なデータも混ざります。これらのデータは向きの判別が出来ないため、学習の際にノイズとなってしまいます。
...
以下を満たす歩行者が一人も存在しない場合、その画像を学習データから除外しました。
...
MEBOWは、人のキーポイント（姿勢）情報などを手がかりに **身体の向き** を推定するモデルです。単眼RGB画像を入力とし、COCO系データを拡張した大規模データで学習されています。

### 今回の立ち位置

本記事では、MEBOWは **学習データ作成用（教師ラベル生成）** としてのみ使用し、推論時には使いません。使用した重みは、公式リポジトリで配布されている **Trained HBOE model** です。
