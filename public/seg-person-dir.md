...
研究要件として、歩行者のセグメンテーションと、向き推定（同方向/逆方向）が必要でした。以前は以下の2モデル構成を想定していました。
...
COCOなどで学習済みの重みを出発点にして、自分のデータと自分のクラス定義に合わせて追加学習することです。

今回やりたいのは「同方向/逆方向」という独自ラベルなので、ファインチューニングで吸収させます。
...
* **学習データ作成用**（教師ラベル生成）
* 推論時には使わない
* 本記事では、公式リポジトリで配布されている **Trained HBOE model** を使用
